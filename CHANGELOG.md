...
- 自动为纯公式和中文句子内的未定界 LaTeX 补充显示或行内公式定界符。
- 将 `\(...\)`、`\[...\]` 和 LaTeX 环境转换为 PillowMD 支持的美元符号定界格式。
- 将单独成行的 `[` / `]` 包围的明确 LaTeX 合并为一个公式块，避免方括号被拆成独立消息。
- 扩充 `\dfrac`、`\le`、`\to`、`\infty` 等常见 LaTeX 命令识别。
...
- 新增 LLM 语境预测、规则回退和随机区间三级调度策略。
- 识别明确延迟、明天、休息、会议、通勤、吃饭、电影和勿扰等时间语境。
- 达到最大未回复次数后停止继续安排主动消息。
- 持久化并展示调度策略、原因、命中规则和来源。
- 新增 AstrBot 原生 Pages 控制台入口及任务状态视图。

## v1.3.0 - 主动消息与智能分段合并版

- 合并主动聊天、普通回复智能分段和媒体组件策略。
- 新增 Markdown 表格、LaTeX 公式、Unicode 数学符号自动识别。
- 富内容在普通文本分段前转为图片，失败时以完整文本块回退。
- 主动消息的所有分段继续使用精确平台路由并写入消息历史。
- 保留原主动消息会话级分段配置，并新增普通回复全局分段配置。
