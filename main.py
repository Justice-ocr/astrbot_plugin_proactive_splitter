...
class ProactiveChatPlugin(
    ...
    UnifiedSplitterMixin,  # 普通/主动消息统一分段与表格、公式转图片
    ...
    async def on_llm_request(self, event: AstrMessageEvent, req) -> None:
        """提示模型使用可稳定识别的 Markdown/LaTeX 格式。"""
        await UnifiedSplitterMixin.unified_on_llm_request(self, event, req)

    @filter.on_llm_response()
    async def on_llm_response(self, event: AstrMessageEvent, resp) -> None:
        """标记模型回复，供仅处理 AI 输出的作用域判断使用。"""
        setattr(event, "__unified_splitter_llm_reply", True)

    @filter.on_decorating_result(priority=-100000000000000000)
    async def on_decorating_result(self, event: AstrMessageEvent) -> None:
        """统一处理普通回复与主动消息的富内容渲染和分段。"""
        await UnifiedSplitterMixin.unified_on_decorating_result(self, event)
