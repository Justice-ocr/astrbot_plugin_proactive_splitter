...
当前合并版仓库：`Justice-ocr/astrbot_plugin_proactive_splitter`。

本目录合并了 `Justice-ocr/astrbot_plugin_proactive_chat`、其上游
`DBJD-CR/astrbot_plugin_proactive_chat` 与 `nuomicici/astrbot_plugin_splitter`。
它保留主动聊天插件的定时任务、
上下文感知、会话级配置、TTS、持久化数据和 Web 管理端，并新增统一的回复处理管线：

- 普通 AI 回复和主动消息共用智能分段逻辑。
- 自动保护括号、引号、代码块和 `<think>` 块，不在内部切断。
- 在分段前识别 Markdown 表格、`$...$`、`$$...$$`、`\(...\)`、`\[...\]`、
  LaTeX 环境以及常见 Unicode 数学符号。
- 表格和公式使用 AstrBot 自带的 Markdown/KaTeX 文转图渲染器生成图片，并作为独立消息发送。
- 渲染失败时回退为完整文本块，不会把表格或公式拆成多个文本分段。
- 主动消息继续沿用各会话的 `segmented_reply_settings`，普通回复使用新增的
  `unified_splitter_settings`。
- 支持通过 LLM 和规则分析最近消息中的时间语境，动态决定下次主动消息时间。
- 达到未回复次数上限后停止续排，并在 AstrBot Pages 中展示任务、调度依据与状态。

安装时请用本合并版替换原来的两个插件，避免多个装饰器同时接管同一条回复。
合并版继续读取原 `astrbot_plugin_proactive_chat` 数据目录，已有会话和任务数据无需手动迁移。
